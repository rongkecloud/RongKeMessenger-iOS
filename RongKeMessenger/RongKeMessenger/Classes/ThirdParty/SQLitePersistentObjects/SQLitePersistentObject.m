...
+(NSArray *)_findByCriteria:(NSString *)criteriaString, ...
{
	...
	[queryString release];
	// NSLog(@"DEBUG: _findByCriteria query = %@", query);
	sqlite3_stmt *statement;
		...
		if (pk < 0)
		{
			...
			if (sqlite3_prepare_v2(database, [pkQuery UTF8String], -1, &statement, nil) == SQLITE_OK) 
			{
				if (sqlite3_step(statement) == SQLITE_ROW) 
				{
					...
					NSString *seqIncrementQuery = [NSString stringWithFormat:@"UPDATE SQLITESEQUENCE set seq=%d WHERE name='%@'", (int)pk, [[self class] tableName]];
...
-(void)_deleteObjectCascade:(BOOL)cascade
{
	...
	NSString *deleteQuery = [NSString stringWithFormat:@"DELETE FROM %@ WHERE pk = %ld", [[self class] tableName], (long)pk];
	sqlite3 *database = [[SQLiteInstanceManager sharedManager] database];
	char *errmsg = NULL;
		...
		if ([colType hasPrefix:@"@"])
		{
			...
			if (isCollectionType(className))
			{
				if (cascade)
				{
					...
					NSString *xRefLoopQuery = [NSString stringWithFormat:@"select fk_table_name, fk from %@_%@ where parent_pk = %ld", [[self class] tableName], [prop stringAsSQLColumnName], (long)pk];
				...
				NSString *xRefDeleteQuery = [NSString stringWithFormat:@"DELETE FROM %@_%@ WHERE parent_pk = %ld",  [[self class] tableName], [prop stringAsSQLColumnName], (long)pk];
				if (sqlite3_exec (database, [xRefDeleteQuery UTF8String], NULL, NULL, &errmsg) != SQLITE_OK)
...
+(void)tableCheck
{
	...
	if (![checkedTables containsObject:[self className]])
	{
        ...
        sqlite3 *database = [[SQLiteInstanceManager sharedManager] database];
        if (database == NULL) {
            [checkedTables removeObject: [self className]];
            return;
        }
...
+ (NSMutableDictionary *)getCacheTableDic
{
    return objectMap;
}

- (void)removeTableCache:(SQLitePersistentObject *)persistentObject
{
    if (persistentObject == nil) {
        return;
    }
    NSString *keyString = [[NSString alloc] initWithFormat: @"%@-%d", [persistentObject className], persistentObject.pk];
    [objectMap removeObjectForKey: keyString];
    [keyString release];
}
