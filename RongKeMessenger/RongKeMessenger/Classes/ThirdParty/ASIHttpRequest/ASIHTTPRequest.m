...
- (void)buildRequestHeaders
{
    ...
    // Gray.Wang:2013.12.07:扩展HTTP Header中：Accept-Language: zh-hans/en/zh-hant/...
    NSString * acceptLanguage = [[[NSLocale preferredLanguages] objectAtIndex:0] lowercaseString];
    // Accept-Language: zh-hans/en/zh-hant/...
    [self addRequestHeader:@"Accept-Language" value:acceptLanguage];
...
- (void)startRequest
{
    ...
    if([[[[self url] scheme] lowercaseString] isEqualToString:@"https"]) {       
        ...
        if (![self validatesSecureCertificate]) {
            ...
            // Gray.Wang:2014.09.22: use xcode6 -fix warning ios4 deprecated
        ...
        if (clientCertificateIdentity) {
            // Gray.Wang:2014.05.13: 表示不校验证书链，否则会报错：CFNetwork SSLHandshake failed (-9807)
...
- (void)readResponseHeaders
{
	...
	if (![self willRedirect]) {
        ...
        // 2013.04.28:修改使用服务器下载的Header中的数据长度为：Accept-Length
        if (cLength == nil) {
            cLength = [responseHeaders valueForKey:@"Accept-Length"];
        }
...
- (void)runPACScript:(NSString *)script
{
	if (script) {
        ...
        // Gray.Wang:2015.09.21: fix warning - proxySettings is _Nonnull
...
+ (void)recordBandwidthUsage
{
    ...
    // Gray.Wang:2014.12.20: Division by zero
    if (measurements > 0) {
